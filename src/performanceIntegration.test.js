...
describe('Performance v3 history + forecast integration', () => {
  ...
  it('keeps precise inspection while removing persistent plot clutter', () => {
    expect(ui).not.toContain('performance-v2-points')
    expect(ui).not.toContain('performance-v2-hover-card')
    expect(ui).toContain('performance-v2-chart-readout')
    ...
    expect(ui).toContain('history-segment')
    expect(ui).toContain('forecast-segment')
    expect(ui).toContain('onPointerLeave={() => setActiveIndex(null)}')
    expect(ui).toContain('monthLabel(active.date, true)')
    expect(styles).toContain('.performance-v2-line.forecast-segment')
    expect(styles).toContain('stroke-dasharray: 7 5')
    expect(styles).toContain('.performance-v2-line.actual-series')
    expect(styles).toContain('.performance-v2-chart-readout')
    expect(styles).toContain('.performance-v2-era-bands .future')
    expect(ui).not.toContain('<rect className="past"')
    expect(ui).not.toContain('>PAST</text>')
    expect(ui).not.toContain('>FORECAST</text>')
  })
