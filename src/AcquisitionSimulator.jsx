...
import { acquisitionCosts, acquisitionJurisdictions, createAcquisition } from './acquisition.js'
...
function Field({ label, prefix, suffix, children }) {
  return <label className="acquisition-field">
    <span>{label}</span>
    <div>
      {prefix && <b>{prefix}</b>}
      {children}
...
function AcquisitionCard({ acquisition, onUpdate, onRemove }) {
  ...
  return <article className="panel acquisition-card">
    <header className="acquisition-card-header">
      <div>
        ...
        <p>
          {[acquisition.postcode, acquisition.bedrooms ? `${acquisition.bedrooms} bed` : '', acquisition.epc ? `EPC ${acquisition.epc}` : '']
            .filter(Boolean).join(' · ') || 'Enter or import the property details below.'}
        </p>
      </div>
      <div className="acquisition-card-actions">
        {acquisition.sourceUrl && <a href={acquisition.sourceUrl} target="_blank" rel="noreferrer" className="secondary-button small">
          Listing <ArrowUpRight size={14} />
        </a>}
        <button type="button" className="icon-button acquisition-remove" aria-label="Remove acquisition" onClick={() => onRemove(acquisition.id)}>
          <Trash2 size={17} />
        </button>
      </div>
    </header>
    ...
    <div className="acquisition-card-layout">
      <div className="acquisition-inputs">
        <section className="acquisition-input-section">
          ...
          <div className="acquisition-fields-grid">
            ...
            <label className="acquisition-text-field">
              <span>Postcode</span>
      ...
      <aside className="acquisition-results">
        ...
        <div className="acquisition-secondary-results">
          ...
          <div><span>Expected gross yield</span><b>{costs.grossYield ? `${(costs.grossYield * 100).toFixed(2)}%` : '—'}</b></div>
        </div>
...
export default function AcquisitionSimulator({ acquisitions, onChange, defaultJurisdiction = 'england-ni' }) {
  ...
  const add = (values = {}) => {
    onChange([...acquisitions, createAcquisition(values, values.jurisdiction || defaultJurisdiction)])
  }
  ...
  const remove = (id) => {
    ...
    onChange(acquisitions.filter((item) => item.id !== id))
  }
