...
const propertyGroups = [
  { title: 'Finance & performance', description: '', tone: 'green', emptyLabel: 'No finance or performance details have been added yet.', emptyEditField: 'latestValuation', rows: [
    ...
    ['Mortgage payment / month', (p) => currency(p.monthlyPayment), 'money-negative', false, 'baseRate', 'Calculated from the effective loan balance and current rate. Interest-only loans show interest; repayment loans include scheduled principal over the remaining mortgage term.', 'mortgagePayment'],
...
function PropertyMetricHelp({ label, help }) {
  if (!help) return null
  return <button type="button" className="property-metric-help" aria-label={`${label}: ${help}`} data-tooltip={help}><CircleHelp size={13} /></button>
}

function PropertyMetricEditButton({ property, label, editField, onEdit }) {
...
function EditDrawer({ property, onSave, onClose, onDelete, isNew, focusField = '' }) {
  ...
  useEffect(() => {
    ...
    if (typeof requestAnimationFrame !== 'function') { focusTarget(); return undefined }
