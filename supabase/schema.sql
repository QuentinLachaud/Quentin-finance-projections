...
create policy "Users can read their own entitlement" on public.account_entitlements
...
begin
  insert into public.account_entitlements (user_id) values (new.id) on conflict (user_id) do nothing;
  return new;
end;
$$;

drop trigger if exists create_account_entitlement on auth.users;
create trigger create_account_entitlement after insert on auth.users
...
begin
  ...
  select coalesce(plan = 'pro', false) into pro_account
  from public.account_entitlements where user_id = new.user_id;
...
create trigger enforce_free_property_limit before insert or update on public.portfolio_states
...
create trigger set_account_entitlements_updated_at before update on public.account_entitlements
for each row execute function public.set_portfolio_updated_at();
